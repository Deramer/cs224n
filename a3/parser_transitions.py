...
class PartialParse(object):
    ...
    def is_finished(self):
        if not len(self.buffer) and len(self.stack) == 1:
            return True
        else:
            return False
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses):
        ...
        for n, part_parse in enumerate(minibatch):
            part_parse.parse_step(transitions[n])
            if part_parse.is_finished():
                unfinished_parses.remove(part_parse)
    dependencies = [part_parse.dependencies for part_parse in partial_parses]
